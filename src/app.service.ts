...
export class AppService {
  ...
  private _now: number;
  private readonly OUTPUT_FILE_PREFIX: string = 'scanOtaStatus_';
  ...
  async execute(): Promise<void> {
    ...
    try {
      ...
      const outputFile = `${this.OUTPUT_FILE_PREFIX}${this._now.toString()}.csv`;
  ...
  private async _terminate(): Promise<void> {
    ...
    const csvFiles = files.filter((x) => x.endsWith('.csv')).filter((x) => x !== `${this.OUTPUT_FILE_PREFIX}${this._now.toString()}.csv`);
